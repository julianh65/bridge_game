...
- [x] I got 9 VP and nothing happened, it didn’t detect that I won and no screen popped up (HUD now surfaces capital safety status.)
- [x] HUD shows capital safe/contested status so the victory gate is visible at a glance.
...
- [x] The aerial faction passive of extra movement didn't work for me
...
- [x] Audit forge collection hard-lock edge case (empty market deck + no scrap candidates).
  - [x] Decide on a backup plan (auto-skip prompt or allow a safe no-op choice) and document it.
