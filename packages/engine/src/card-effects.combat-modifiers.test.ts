...
describe("combat card effects", () => {
  ...
  it("gold plated armor spends gold to prevent champion damage", () => {
    vi.spyOn(shared, "rollDie").mockImplementation((rng) => ({ value: 1, next: rng }));
    ...
    board.units = {
      c1: createChampion("c1", "p1", hexKey, 1),
      c2: { ...createChampion("c2", "p2", hexKey, 1), bounty: 0 }
    };
    ...
    expect(protectedUnit.hp).toBe(1);
