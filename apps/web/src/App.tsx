        ...
        <div className="layout">
                ...
                {debug.mineKeys.map((key) => (
                  <li key={key}>
                    ...
                    {debug.board.hexes[key].mineValue ? (
                      <span className="chip">{debug.board.hexes[key].mineValue}</span>
                    ) : null}
                  </li>
                ))}
              </ul>
          ...
          <section className="panel board">
            <div className="legend">
              <span className="legend__item legend__item--capital">Capital</span>
              <span className="legend__item legend__item--forge">Forge</span>
              <span className="legend__item legend__item--mine">Mine</span>
              <span className="legend__item legend__item--center">Center</span>
            </div>
            <svg
              viewBox={`${viewBox?.minX ?? 0} ${viewBox?.minY ?? 0} ${viewBox?.width ?? 0} ${
                viewBox?.height ?? 0
              }`}
            >
              {debug.hexRender.map((hex) => (
                <g key={hex.key} className={`hex hex--${hex.tile}`}>
                  <polygon points={hexPoints(hex.x, hex.y, HEX_SIZE)} />
                  <text x={hex.x} y={hex.y + 4} className="hex__label">
                    {hex.mineValue ?? ""}
                  </text>
                </g>
              ))}
