...
## Active tasks
- owner: agent5 | scope: add player-target support + Market Squeeze steal-gold card (engine + UI) | files: packages/engine/src/card-effects.ts, packages/engine/src/card-effects-targets.ts, packages/engine/src/card-effects-economy.ts, packages/engine/src/content/cards/age3.ts, apps/web/src/components/GameScreen.tsx, apps/web/src/components/GameScreenHandPanel.tsx, implementation_plan.md, progress.md | status: in progress (overlap: card-effects*.ts + implementation_plan.md with agent4)
## Planning updates
