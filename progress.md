...
- (owner: agent2) Add capital hover labels to show the owning faction (e.g., "Prospect Capital"); scope board tooltips/labels + capital owner mapping; planned files: `apps/web/src/components/GameScreen.tsx`, `apps/web/src/components/BoardView.tsx`, `implementation_plan.md`, `progress.md`. (status: in progress)
...
- Added one basic champion card per age (Stoneguard, Iron Sentinel, Obsidian Sentinel), updated rules counts, and marked the plan item complete. (Files: `packages/engine/src/content/cards/age1.ts`, `packages/engine/src/content/cards/age3.ts`, `rules_draft.md`, `implementation_plan.md`.) (owner: agent2)
- Expanded the free starting card pool with additional Age I non-champion options. (File: `packages/engine/src/config.ts`.) (owner: agent2)
...
- Added Card Editor to the Debug dropdown so debug views live under one menu. (File: `apps/web/src/App.tsx`.) (owner: agent5)
...
- Allowed forge reforge to scrap from hand/draw/discard, with updated collection UI/validation and deck-scrap handling in the engine; updated rules/spec + plan checklist. (Files: `packages/engine/src/cards.ts`, `packages/engine/src/round-flow.ts`, `apps/web/src/components/CollectionPanel.tsx`, `apps/web/src/components/GameScreenOverlays.tsx`, `apps/web/src/components/GameScreen.tsx`, `rules_draft.md`, `technical_spec.md`, `implementation_plan.md`.) (owner: agent4)
- Coordinated Advance now moves up to 2 stacks (not forced 2), with matching rules text; marked the plan item complete. (Files: `packages/engine/src/content/cards/age2.ts`, `rules_draft.md`, `implementation_plan.md`.) (owner: agent3)
...
- Added mine yield to the tile hover tooltip so mine value stays visible when stacks cover the hex. (File: `apps/web/src/components/BoardView.tsx`.) (owner: agent2)
...
- Fixed Forge Sketch optional discard handling (engine + UI), gated draw on discard, and added test coverage; updated the plan checklist. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/card-effects-economy.ts`, `packages/engine/src/content/cards/power.ts`, `packages/engine/src/card-effects.forge-sketch.test.ts`, `apps/web/src/components/GameScreen.tsx`, `implementation_plan.md`.) (Test: `npm run -w @bridgefront/engine test -- card-effects.forge-sketch.test.ts`.) (owner: agent1)
