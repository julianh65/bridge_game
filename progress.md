...
- Added combat sync stages so both players click to reveal hit assignments and resolve hits before advancing to the next round; updated server sync flow and overlay logic, and checked off the remaining combat overlay flow items. (Files: `apps/server/src/server.ts`, `apps/web/src/components/CombatOverlay.tsx`, `apps/web/src/lib/room-client.ts`, `implementation_plan.md`.)
...
- Added desaturated card-art backplates to force stacks and champion tokens (champions use their art; forces use a generic recruit art). (Files: `apps/web/src/components/BoardView.tsx`, `apps/web/src/styles.css`, `implementation_plan.md`.) (Overlap note: `apps/web/src/components/BoardView.tsx`, `apps/web/src/styles.css` have other in-flight edits.)
...
- Swapped forge scrap selection from ID pills to card previews in the collection panel; marked the plan item complete. (Files: `apps/web/src/components/CollectionPanel.tsx`, `implementation_plan.md`.)
...
- Refined combat overlay champion token art with a dedicated art layer + desaturated filter so the modal matches board chits. (Files: `apps/web/src/components/CombatOverlay.tsx`, `apps/web/src/styles.css`.)
...
- Implemented server-synced market roll-off clicks with a roll-off block/command, auto-resolve handling, and UI roll buttons; updated market/smoke tests and checked off the plan item. (Files: `packages/engine/src/types.ts`, `packages/engine/src/market.ts`, `packages/engine/src/engine.ts`, `packages/engine/src/round-flow.ts`, `packages/engine/src/market.test.ts`, `packages/engine/src/smoke.test.ts`, `apps/web/src/components/MarketPanel.tsx`, `apps/web/src/components/GameScreen.tsx`, `apps/web/src/App.tsx`, `apps/server/src/server.ts`, `implementation_plan.md`.) (Overlap note: `apps/web/src/App.tsx` already had in-flight debug overlay wiring in the working tree.)
...
- Note: commit `f84cd33` picked up pre-existing `apps/web/src/components/BoardView.tsx` edits from the working tree (not part of the move-split task).
...
- Added Precise Planning (Age I) with draw-pile-empty conditional draw + mana effects and updated the plan checklist. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/card-effects-economy.ts`, `packages/engine/src/content/cards/age1.ts`, `implementation_plan.md`.) (Overlap note: this commit also includes agent2 plan/progress updates for the Spoils of War test.)
- Added Spellcaster (Age I) with conditional spell draw, plus spellcaster effect resolution and refined draw-pile-empty checks. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/card-effects-economy.ts`, `packages/engine/src/content/cards/age1.ts`, `implementation_plan.md`.) (Overlap note: `card-effects*.ts` may overlap agent5 Market Squeeze work.)
- Added Propaganda Recruitment + Future Investment (Age I) with scaling counters, discard triggers, and recruit scaling; marked the plan item complete. (Files: `packages/engine/src/player-flags.ts`, `packages/engine/src/cards.ts`, `packages/engine/src/round-flow.ts`, `packages/engine/src/card-effects-units.ts`, `packages/engine/src/content/cards/types.ts`, `packages/engine/src/content/cards/age1.ts`, `implementation_plan.md`.)
...
- Added discard/burn-from-hand card effects with hand picker selection support, plus Age II Conscription Drive + Clean Cuts card defs; updated the plan checklist. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/content/cards/age2.ts`, `apps/web/src/components/GameScreen.tsx`, `implementation_plan.md`.) (owner: agent4)
- Added Age II Bridge Pivot card def and engine effect to swap a bridge for a new adjacent bridge; updated the plan checklist. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/content/cards/age2.ts`, `implementation_plan.md`.) (owner: agent4)
...
- Added Cycle Protocol (Age II) deckcraft card def with draw 3/discard 2 and updated the plan checklist. (Files: `packages/engine/src/content/cards/age2.ts`, `implementation_plan.md`.)
- Added Smuggling Ring (Age II) with enemy-capital conditional gold effect and targeted tests; updated the plan checklist. (Files: `packages/engine/src/card-effects.ts`, `packages/engine/src/content/cards/age2.ts`, `packages/engine/src/card-effects.smuggling-ring.test.ts`, `implementation_plan.md`.) (Test: `npm run -w @bridgefront/engine test -- card-effects.smuggling-ring.test.ts`.) (owner: agent2)
- Added Guild Favor card-effect test for gold + draw and updated the plan checklist. (Files: `packages/engine/src/card-effects.guild-favor.test.ts`, `implementation_plan.md`.) (Test: `npm run -w @bridgefront/engine test -- card-effects.guild-favor.test.ts`.) (owner: agent2)
...
- Added Spoils of War card-effect test for battle-win gold (plan already tracked). (File: `packages/engine/src/card-effects.combat-modifiers.test.ts`.) (Test: not run.) (owner: agent2)
...
- Treated selected build-bridge edges as temporary bridges during card move targeting so Bridge Crew/Bridge Deed can move across the new edge; updated the plan checklist. (Files: `apps/web/src/components/GameScreen.tsx`, `implementation_plan.md`.) (owner: agent4)
...
- Added hex-pair targeting support for wormhole cards with a two-step board pick flow, target hints, and a dashed link preview between chosen hexes. (Files: `apps/web/src/components/ActionPanel.tsx`, `apps/web/src/components/GameScreen.tsx`, `apps/web/src/components/GameScreenHandPanel.tsx`, `apps/web/src/components/BoardView.tsx`, `apps/web/src/components/GameScreenBoardSection.tsx`, `apps/web/src/styles.css`, `implementation_plan.md`.) (owner: agent2)
...
- Added an available-slots count to the capital draft summary to make remaining picks clearer. (File: `apps/web/src/components/SetupCapitalDraft.tsx`.) (owner: agent2)
...
- Marked conditional economy triggers and bridge manipulation suite parent tasks as complete in the plan. (File: `implementation_plan.md`.)
