...
- [owner: agent1] [status: in-progress] Add combat sync stages so players click to reveal hit assignments and resolve hits before advancing rounds. Files: `apps/server/src/server.ts`, `apps/web/src/lib/room-client.ts`, `apps/web/src/components/CombatOverlay.tsx`, `implementation_plan.md`, `progress.md`. (Overlap note: avoid touching `apps/web/src/components/GameScreen.tsx` while agent5 is active.)
## Planning updates
